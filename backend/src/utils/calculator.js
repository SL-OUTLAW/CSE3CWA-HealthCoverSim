...
function cal_quote(data) {
  ...
  let lhc1 = cal_lhc(data.age1, data.history1, data.hospital_cover);

  let a1_cost = HOSPITAL_PRICES[data.hospital_cover] * (1 + lhc1);
  ...
  if (adults == 2) {
    ...
    lhc2 = cal_lhc(data.age2, data.history2, data.hospital_cover);

    a2_cost = HOSPITAL_PRICES[data.hospital_cover] * (1 + lhc2);
  }
